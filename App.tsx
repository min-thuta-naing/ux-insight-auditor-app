...
// --- View Types ---
type ViewState = 'LANDING' | 'STUDENT_ONBOARDING' | 'AUDITOR' | 'SUBMISSION_SUCCESS' | 'PROF_LOGIN' | 'PROF_DASHBOARD';
...
const App: React.FC = () => {
  ...
  // Used by Professor to load a student's work
  const handleLoadSubmission = (sub: StudentSubmission) => {
      ...
      // Keep student name in context if needed, but switch to auditor view
      // We assume prof wants to see it in the auditor interface
  ...
  const handleLoadAudit = (audit: SavedAudit) => {
    ...
    setError(null);
    setCurrentView('AUDITOR'); // Automatically switch to auditor view
  };
          ...
          <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-4">
              ...
              <div className="max-w-4xl w-full text-center space-y-8">
                  ...
                  <div className="grid md:grid-cols-2 gap-8 mt-12">
                      ...
                      {/* Professor Card */}
                      <div className="bg-white p-8 rounded-2xl shadow-lg border-2 border-transparent hover:border-violet-500 transition-all cursor-pointer group" onClick={() => setCurrentView('PROF_LOGIN')}>
                          ...
                          <button className="mt-6 w-full py-2 bg-violet-600 text-white rounded-lg font-semibold hover:bg-violet-700">Instructor Login</button>
                      </div>
                  </div>
              ...
              <div className="bg-white p-8 rounded-xl shadow-xl max-w-md w-full">
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="pt-4 flex flex-col gap-3">
                          ...
                          {savedAudits.length > 0 && (
                            <button onClick={() => setIsHistoryOpen(true)} className="text-xs text-center text-indigo-600 hover:underline mt-2">
        ...
        <div className="min-h-screen bg-green-50 flex items-center justify-center p-4">
            <div className="bg-white p-10 rounded-2xl shadow-xl max-w-lg w-full text-center">
                ...
                <div className="flex gap-4">
                     <button onClick={() => {
                         // Reset session for next user or same user new task
                         setReports([]);
                         setSelectedImage(null);
                         setCurrentView('LANDING');
                     }} className="flex-1 py-3 border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-50 font-medium">Return Home</button>
                     <button onClick={() => {
                         // Reset audit but keep student session
                         setReports([]);
                         setSelectedImage(null);
                         setCurrentView('AUDITOR');
                     }} className="flex-1 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium">New Audit</button>
                </div>
            </div>
        </div>
      );
        ...
        <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
            <div className="bg-white p-8 rounded-xl shadow-2xl max-w-sm w-full">
                <div className="text-center mb-8">
                    <h2 className="text-2xl font-bold text-slate-900">Instructor Login</h2>
                    <p className="text-slate-500 text-sm">Access the analyst dashboard</p>
                </div>
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Password</label>
                        <input type="password" value={profPassword} onChange={e => setProfPassword(e.target.value)} className="w-full border rounded-md px-3 py-2 focus:ring-2 focus:ring-violet-500 outline-none" placeholder="Enter 'admin'" />
                    </div>
                    <button 
                        onClick={() => {
                            if (profPassword === 'admin') setCurrentView('PROF_DASHBOARD');
                            else alert('Incorrect password (hint: admin)');
                        }} 
                        className="w-full py-2 bg-violet-600 text-white rounded-lg font-semibold hover:bg-violet-700"
                    >
                        Login
                    </button>
                    <button onClick={() => setCurrentView('LANDING')} className="w-full text-sm text-slate-500 hover:text-slate-800">Back to Home</button>
                </div>
            </div>
        </div>
      );
              ...
              <header className="bg-slate-900 text-white shadow-md">
                <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="px-2 py-0.5 bg-slate-700 rounded text-xs text-slate-300">HW-01</span>
                    </div>
                    <div className="flex gap-4">
                        <button onClick={handleExport} className="text-sm bg-slate-700 hover:bg-slate-600 px-3 py-1 rounded">Export CSV</button>
                        <button onClick={() => setCurrentView('LANDING')} className="text-sm text-slate-400 hover:text-white">Logout</button>
                    </div>
                </div>
              </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
            ...
            <div className="col-span-1 md:col-span-2">
               ...
               <select
                ...
                className="block w-full rounded-md border-slate-300 py-2 pl-3 pr-8 text-base focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm bg-slate-50 border"
               >
            ...
            {/* WCAG Level - 2 Columns (Visible only if Inclusive) */}
            ...
            {/* Persona - 2 Columns (Always Visible) */}
            <div className="col-span-1 md:col-span-2">
            ...
            {/* Heuristic Selector - Flexible Width */}
