        ...
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
          {findings.map((f) => {
            ...
            return (
              <div
                key={f.id}
                className="absolute transition-all duration-200 pointer-events-auto cursor-pointer hover:bg-white/10 group"
                style={{
                  top: `${top}%`,
                  left: `${left}%`,
                  width: `${width}%`,
              ...
              >
                 <span 
                    className={`absolute -top-6 left-0 text-xs px-1 rounded text-white font-bold opacity-0 group-hover:opacity-100 transition-opacity ${isSelected ? 'opacity-100' : ''}`}
